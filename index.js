// index.js (Truly Complete & Final - with Robust Message Handling)
...
async function handleReceiptSubmission(sender_psid, imageUrl) {
    ...
    // ONLY analyze the image if the user is in the correct state
    if (userState && userState.state === 'awaiting_payment_receipt') {
        ...
        // If they send an image at any other time, just show the menu
        await sendText(sender_psid, "If you're trying to submit a receipt, please start the purchase process first by typing '1'.");
        if (isAdmin) {
            await adminHandler.showAdminMenu(sender_psid, sendText);
        } else {
            await userHandler.showUserMenu(sender_psid, sendText);
        }
    }
}

// --- NEW, ROBUST MAIN MESSAGE ROUTER ---
async function handleMessage(sender_psid, webhook_event) {
    ...
    if (message && message.text) {
        ...
        // Route to handlers based on admin status and state
        if (isAdmin) {
    ...
    else if (message && message.attachments && message.attachments[0].type === 'image') {
        const imageUrl = message.attachments[0].payload.url;
        await handleReceiptSubmission(sender_psid, imageUrl);
    } 
    // --- 3. Handle Other Message Types (like stickers) ---
    else {
        console.log(`Received non-text/non-image message from ${sender_psid}. Showing menu.`);
        return isAdmin ? adminHandler.showAdminMenu(sender_psid, sendText) : userHandler.showUserMenu(sender_psid, sendText);
    }
}
