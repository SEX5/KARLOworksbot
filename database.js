// database.js (Corrected & Enhanced)
...
async function setupDatabase() {
    ...
    try {
        ...
        // UPDATED: Added 'lang' column to creation_jobs
        await client.query(`CREATE TABLE IF NOT EXISTS creation_jobs ( 
...
// NEW: Required by job_poller.js
async function getActionableJobs() {
    const res = await getDb().query(`SELECT * FROM creation_jobs WHERE status IN ('completed', 'failed') ORDER BY updated_at ASC`);
    return res.rows;
}

// NEW: Required by job_poller.js
async function getStalePendingJobs(minutes = 20) {
    const res = await getDb().query(`SELECT * FROM creation_jobs WHERE status = 'pending' AND created_at < NOW() - INTERVAL '$1 minutes'`, [minutes]);
    return res.rows;
}
...
async function updateJobStatus(jobId, newStatus, resultMessage = null) {
    await getDb().query(`UPDATE creation_jobs SET status = $1, result_message = $2, updated_at = CURRENT_TIMESTAMP WHERE job_id = $3`, [newStatus, resultMessage, jobId]);
}

// UPDATED: Added 'lang' parameter handling
...
// NEW: Required for language detection in index.js
...
async function setMaintenanceStatus(isMaintenance) {
    await getDb().query("UPDATE app_settings SET value = $1 WHERE key = 'maintenance_mode'", [isMaintenance ? 'true' : 'false']);
}
