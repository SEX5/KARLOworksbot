// database.js (FINAL, COMPLETE, AND CORRECTED)
...
async function setupDatabase() {
    ...
    try {
        ...
        // Updated Table Schema to include lang
        ...
        // Verification of columns for existing tables
        try { await client.query('ALTER TABLE admins ADD COLUMN is_online BOOLEAN DEFAULT FALSE'); } catch (e) { if (e.code !== '42701') throw e; }
        try { await client.query('ALTER TABLE mods ADD COLUMN x_coordinate REAL'); await client.query('ALTER TABLE mods ADD COLUMN y_coordinate REAL'); } catch (e) { if (e.code !== '42701') throw e; }
        try { await client.query('ALTER TABLE "references" ADD COLUMN last_replacement_timestamp TIMESTAMPTZ'); } catch (e) { if (e.code !== '42701') throw e; }
        try { await client.query('ALTER TABLE users ADD COLUMN lang TEXT DEFAULT \'en\''); } catch (e) { if (e.code !== '42701') throw e; }
        
        // Ensure lang column exists in creation_jobs for proper delivery
        try { await client.query('ALTER TABLE creation_jobs ADD COLUMN lang TEXT DEFAULT \'en\''); console.log('Verified "lang" column in creation_jobs.'); } catch (e) { if (e.code !== '42701') throw e; }
...
async function getActionableJobs() {
    // Fetches jobs that have been processed by the worker but not yet finalized by the bot
...
async function getStalePendingJobs(minutes = 20) {
    // Checks for jobs stuck in 'pending' for too long (worker might be offline)
...
async function pauseUser(userId) { 
    const res = await getDb().query('INSERT INTO paused_users (user_id) VALUES ($1) ON CONFLICT (user_id) DO NOTHING', [userId]); 
    return res.rowCount; 
}

async function resumeUser(userId) { 
    const res = await getDb().query('DELETE FROM paused_users WHERE user_id = $1', [userId]); 
    return res.rowCount; 
}
...
// --- Mod & Account Management ---
async function getMods() { 
    const res = await getDb().query('SELECT m.id, m.name, m.description, m.price, m.image_url, m.default_claims_max, (SELECT COUNT(*) FROM accounts WHERE mod_id = m.id AND is_available = TRUE) as stock FROM mods m ORDER BY m.id'); 
    return res.rows; 
}
...
async function addMod(id, name, description, price, imageUrl, defaultClaimsMax) { 
    await getDb().query('INSERT INTO mods (id, name, description, price, image_url, default_claims_max) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT(id) DO NOTHING', [id, name, description, price, imageUrl, defaultClaimsMax]); 
}
...
// --- Reference & Claim Management ---
async function getAllReferences() { 
    const res = await getDb().query('SELECT r.ref_number, r.user_id, r.claims_used, r.claims_max, m.name as mod_name FROM "references" r JOIN mods m ON r.mod_id = m.id ORDER BY r.timestamp DESC'); 
    return res.rows; 
}

async function getReference(refNumber) { 
    const res = await getDb().query('SELECT r.*, m.name as mod_name FROM "references" r JOIN mods m ON r.mod_id = m.id WHERE r.ref_number = $1', [refNumber]); 
    return res.rows[0] || null; 
}
...
async function updateReferenceClaims(refNumber, claimsUsed, claimsMax) { 
    const res = await getDb().query('UPDATE "references" SET claims_used = $1, claims_max = $2 WHERE ref_number = $3', [claimsUsed, claimsMax, refNumber]); 
    return res.rowCount; 
}
...
async function getSalesStatistics(period) {
    ...
    `;
    const days = interval.split(' ')[0];
    const res = await getDb().query(query, [days]);
    return res.rows;
}
