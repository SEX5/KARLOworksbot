...
async function setupDatabase() {
    ...
    try {
        ...
        try { await client.query('ALTER TABLE admins ADD COLUMN is_maintenance_mode BOOLEAN DEFAULT FALSE'); console.log('Verified "is_maintenance_mode" column in admins table.'); } catch(e) { if (e.code !== '42701') { throw e; } }
...
async function getStalePendingJobs(minutes = 20) {
    const query = ` SELECT job_id FROM creation_jobs WHERE status = 'pending' AND created_at < NOW() - INTERVAL '${minutes} minutes' `;
    const res = await getDb().query(query);
    return res.rows;
}
...
async function setMaintenanceMode(isMaintenance) {
    await getDb().query('UPDATE admins SET is_maintenance_mode = $1', [isMaintenance]);
}
...
async function updateReferenceClaims(refNumber, newClaimsUsed, newClaimsMax) {
    const query = 'UPDATE "references" SET claims_used = $1, claims_max = $2 WHERE ref_number = $3';
    const res = await getDb().query(query, [newClaimsUsed, newClaimsMax, refNumber]);
    return res.rowCount;
}
...
async function deleteAvailableAccountsByModId(modId) {
    const query = 'DELETE FROM accounts WHERE mod_id = $1 AND is_available = TRUE';
    const res = await getDb().query(query, [modId]);
    return res.rowCount;
}
...
module.exports = {
    ...
    pauseUser,
    resumeUser,
    deleteAvailableAccountsByModId,
    getAllUserPSIDs
};
