...
async function setMaintenanceMode(isMaintenance) { await getDb().query('UPDATE admins SET is_maintenance_mode = $1', [isMaintenance]); }
...
async function updateReferenceClaims(refNumber, newClaimsUsed, newClaimsMax) { const query = 'UPDATE "references" SET claims_used = $1, claims_max = $2 WHERE ref_number = $3'; const res = await getDb().query(query, [newClaimsUsed, newClaimsMax, refNumber]); return res.rowCount; }
...
module.exports = { setupDatabase, getActionableJobs, updateJobStatus, getStalePendingJobs, deleteReference, setAdminOnlineStatus, setMaintenanceMode, createAccountCreationJob, getCreationJobs, isAdmin, getAdminInfo, updateAdminInfo, getAllReferences, addBulkAccounts, updateModDetails, updateReferenceMod, updateReferenceClaims, addReference, getMods, getModById, getReference, getAvailableAccount, claimAccount, useClaim, addMod, getModsByPrice, addBulkReferences, isUserPaused, pauseUser, resumeUser, deleteAvailableAccountsByModId, getAllUserPSIDs }; 
